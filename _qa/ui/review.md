...
4. **P1 / 进度 / 主线成绩污染排行榜**：排行榜读取与冠军入口从首局可用，保存只在无尽模式启用；主线结算使用关卡秒数和任务胡萝卜，不提交距离。
...
8. **无尽入口审计**：默认 URL 直接进入无尽、HUD 为距离且任务行为空；商店从第 1 局起显示模式切换。只有玩家主动选择主线时 URL 才写入 `mode=campaign`。
...
1. **P1 / 默认入口 / 有限主线遮蔽排行榜跑分**：移除“第 3 关后解锁无尽”的入口条件；普通 URL 在 stage 1 直接显示 `Distance`，任务行为空，角色商店模式按钮显示“Return to campaign”。
