/*
 * Visual engine adapted from "The frantic run of the valorous rabbit"
 * by Karim Maaloul (Yakudoo), CodePen YGxYej, MIT License.
 * The recognizable model boundaries, camera, palette, motion formulae, and
 * spherical-ground construction are intentionally retained for visual parity.
 */
...
const TweenMax = {
  to(target, duration, vars) {
    return gsap.to(target, { duration, ...vars });
  },
  from(target, duration, vars) {
    return gsap.from(target, { duration, ...vars });
  },
  killTweensOf(target) {
...
function handleWindowResize() {
  if (!renderer || !containerElement) return;
  HEIGHT = containerElement.clientHeight || window.innerHeight;
  WIDTH = containerElement.clientWidth || window.innerWidth;
  windowHalfX = WIDTH / 2;
  windowHalfY = HEIGHT / 2;
...
function gameOver(){
  ...
  TweenMax.to(speedTween, 1, {
    value: 0,
    onUpdate: function() { speed = speedTween.value; },
  });
  TweenMax.to(camera.position, 3, {
    z: baselineMode ? cameraPosGameOver : 205,
    y: baselineMode ? 60 : 48,
    x: baselineMode ? -30 : -18
  });
...
function createFirs(){
  ...
  var nTrees = 100;
   for(var i=0; i< nTrees; i++){
    ...
    theta += baselineMode && Math.random() <= .05
      ? - .35 - Math.random()*.1
      : .25 + Math.random()*.3;
...
function getBonus(){
  ...
  carrotCount += 1;
  callbacks.onBonus?.({ carrots: carrotCount });
...
function getMalus(){
  ...
  if (!reducedMotion) {
    ...
    TweenMax.to(flash, .5, {
      alpha: 0,
      onUpdate:function(){
        renderer.setClearColor(malusClearColor, flash.alpha);
      }
    })
  }
}
...
function updateDistance(){
  ...
  var d = distance/2;
  callbacks.onDistance?.(Math.floor(d));
}
...
export async function createRabbitWorld(element, options = {}) {
  ...
  return {
    jump() {
      if (gameStatus !== 'play') return false;
      var wasRunning = hero.status === 'running';
      hero.jump();
      return wasRunning;
    },
    replay() {
      if (gameStatus !== 'readyToReplay') return false;
      replay();
      return true;
    },
    setPaused(paused) {
    ...
    destroy() {
      ...
      if (typeof TweenMax.killAll === 'function') TweenMax.killAll(false, true, true, true);
